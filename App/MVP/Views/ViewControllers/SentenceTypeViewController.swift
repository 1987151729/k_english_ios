//
//  SentenceTypeViewController.swift
//  App
//
...
class SentenceTypeViewController: UIViewController {
    
    // 属性定义
    // 非视图属性
    var presenter: SentenceTypePresenter?
    var dataList: [SentenceType]!
...
extension SentenceTypeViewController {
    ...
    func initBody() {
        ...
        self.uCollectionView?.register(SentenceTypeCollectionViewCell.self, forCellWithReuseIdentifier: forCellReuseIdentifier)
...
extension SentenceTypeViewController {
    ...
    func initData(){
        presenter = SentenceTypePresenter.init(viewProtocol: self)
...
// 事件处理（非数据业务）[实现协议 ]
extension SentenceTypeViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let uCell = collectionView.dequeueReusableCell(withReuseIdentifier: forCellReuseIdentifier, for: indexPath) as! SentenceTypeCollectionViewCell
        
        uCell.uLabelName.text = sentenceType.name
        uCell.uLabelDescription.text = sentenceType.description
        if 0 == indexPath.row {
...
// 事件处理（数据业务）[实现协议 ]
extension SentenceTypeViewController: SentenceTypeViewProtocol {
    ...
    func refresh(sentenceTypeList: [SentenceType]) {
        dataList.removeAll()
        dataList = dataList + sentenceTypeList
        // 重新加载表格数据
