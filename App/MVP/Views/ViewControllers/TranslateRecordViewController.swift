//
//  TranslateRecordViewController.swift
//  App
//
...
class TranslateRecordViewController: UIViewController {
    
    // 属性定义
    // 非视图属性
    private var presenter: TranslateRecordViewPresenter?
    private var dataList: [TranslateRecord]!
...
// 界面布局
extension TranslateRecordViewController {
    ...
    private func initNavigation() {
        // 标题
        self.navigationItem.title = NSLocalizedString("translate_record_title", comment: "")
        // 左按钮
    ...
    private func initBody() {
        ...
        self.uTableView!.register(TranslateRecordTableViewCell.self, forCellReuseIdentifier: forCellReuseIdentifier)
...
extension TranslateRecordViewController {
    ...
    private func initData(){
        presenter = TranslateRecordViewPresenter.init(viewProtocol: self)
        dataList = [TranslateRecord]()
        headerRefresh()
    }
}
...
extension TranslateRecordViewController{
    ...
    // 播放
    private func play(translateRecord: TranslateRecord){
        var url: String?
        if TranslateRecordType.EnToCh.rawValue == translateRecord.type {
            url = translateRecord.speakUrl
        }
        else{
            url = translateRecord.tSpeakUrl
        }
...
// 事件处理（非数据业务）[实现协议 ]
extension TranslateRecordViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath)
        -> UITableViewCell {
            let translateRecord = self.dataList[indexPath.row]
            // 为了提供表格显示性能，已创建完成的单元需重复使用
            // 同一形式的单元格重复使用，在声明时已注册
            var uCell: TranslateRecordTableViewCell = tableView.dequeueReusableCell(withIdentifier: forCellReuseIdentifier, for: indexPath) as! TranslateRecordTableViewCell
            
            if uCell.isEqual(nil) {
                uCell = TranslateRecordTableViewCell(style: .default, reuseIdentifier: forCellReuseIdentifier)
            }
            //            cell.accessoryType = .disclosureIndicator
            uCell.selectionStyle = .none
            
            uCell.uLabelQuery.text = translateRecord.query
            uCell.uLabelTranslation.text = translateRecord.translation
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath){
        let translateRecord = self.dataList[indexPath.row]
        self.play(translateRecord: translateRecord)
    }
}

// 事件处理（数据业务）[实现协议 ]
extension TranslateRecordViewController: TranslateRecordViewProtocol {
